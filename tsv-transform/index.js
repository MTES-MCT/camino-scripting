...
function printTitresPoints(pointsFile) {
  pointsFile.points.forEach(e => {
    console.log([
      `${pointsFile.etape}-g${e.groupe.padStart(2, '0')}-c${e.contour.padStart(2, '0')}-p${e.point.padStart(3, '0')}`,
      `"${e.coordonnees}"`,
      e.groupe,
      e.contour,
      e.point,
...
function printTitresPointsReferences(pointsFile) {
  pointsFile.points.forEach(e => {
    const id = `${pointsFile.etape}-g${e.groupe.padStart(2, '0')}-c${e.contour.padStart(2, '0')}-p${e.point.padStart(3, '0')}`;
...
function parseFile(content) {
  const points = content.trim().replace(/\r/g, '').split('\n').slice(1).map(e => {
    // console.log(e)

    let [groupe, contour, point, jorf_id, description, coord1, coord2] = e.split('\t')
    console.log(e)
    console.log({ coord1, coord2 })
  ...
  first.point = (new Number(last.point) + 1) + ''
...
async function main() {
  ...
  files = files.filter(f => f.match(/poses-lery/)).filter(f => f.match(/oct/)).filter(f => !f.match(/old/))

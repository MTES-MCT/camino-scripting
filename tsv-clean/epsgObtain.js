...
const EpsgCreate = () => {
  const li = ['w', 'c', 'g', 'h', 'm']
  return li.map(EpsgFolderCreate)
}
...
const matEpsg = EpsgCreate()
const newMatEpsg = EpsgModify(matEpsg)

fs.writeFileSync(`${filespath}dataTsv.json`, JSON.stringify(newMatEpsg))
process.exit()

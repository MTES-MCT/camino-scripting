...
function explode(obj, prefix) {
  return (
    obj &&
    Object.keys(obj).reduce((r, k) => ((r[`${prefix}_${k}`] = obj[k]), r), {})
  )
}
...
async function main() {
  ...
  const demandeursIndex = demandeurs.reduce(
    (r, d) => ((r[d.idd_demandeurs] = d), r),
    {}
  )
  ...
  const res = gda.reduce((r, gdaTitre) => {
    ...
    gdaTitre.demandeur = (
      demandeursIndex[gdaTitre.idd_demandeurs] || {}
    ).societe

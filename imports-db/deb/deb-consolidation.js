...
const names = ['titres', 'titulaires', 'demarches', 'etapes']
...
async function main() {
  ...
  const joined = result[0].data
    ...
    .filter(title => {
      const geo = geos.find(
        f => f.idtm.toLowerCase() === title.num_titre.toLowerCase()
      )
  ...
  if ('consolidation') {
    ...
    const fields = [
      ...
      'titulaire',
      'deb.idd_demandeurs',
      'deb.demandeur.societe',
      'deb.demandeur.siret',
      'date_oct',
      'deb.date_octroi',
      'dateperemp',
      'deb.date_echeance',
      'deb.date_renonciation',
      'subst_1',
      'subst_2',
      'surf_off',
      'deb.surface',

...
function buildTitreRef(titre) {
  return `${(titre.datprefsd || 'XXXX').slice(0, 4)}-${titre.cod.padEnd(
    4,
    '0'
  )}-${titre.txtcod}`
}
...
async function buildTitres() {
  ...
  const indexDemarches = demarches.reduce(
    (r, t) => ((r[t.id_tit] = (r[t.id_tit] || []).concat(t)), r),
    {}
  )
  let demarchesTypes = await csv().fromFile(
    '../sources/csv/deb_affaires_types.csv'
  )
  const indexDemarchesTypes = demarchesTypes.reduce(
    (r, t) => ((r[t.id] = t), r),
    {}
  )

  let etapes = await csv().fromFile('../sources/csv/deb_events.csv')
  const indexEtapes = etapes.reduce(
  ...
  let documents = await csv().fromFile(
    '../sources/csv/deb_events_documents.csv'
  )
  ...
  const indexLiens = liens.reduce(
    (r, e) => ((r[e.id_tit] = indexTitulaires[e.id_titu]), r),
    {}
  )

  let substances = await csv().fromFile('../sources/csv/deb_substances.csv')
  const indexSubstances = substances.reduce((r, t) => ((r[t.id] = t), r), {})
  let titresSubstances = await csv().fromFile(
    '../sources/csv/deb_titres_substances.csv'
  )
  const indexTitresSubstances = titresSubstances.reduce(
    (r, t) => ((r[t.id_tit] = (r[t.id_tit] || []).concat(t)), r),
    {}
  )
  ...
  titres = titres.map(titre => {
    let { itv_rfa: siret = '', itv_nom_lb: titulaire = '' } =
      indexLiens[titre.id] || {}
    ...
    if (titresSubstances && titresSubstances.length) {
      substances = titresSubstances
        .map(s => (indexSubstances[s.id_sub] || {}).nom_sub)
        .join(', ')
    }
...
async function buildTitulaires() {
  ...
  let titulaires = await csv().fromFile(
    '../sources/csv/deb_liens_titulaires.csv'
  )
